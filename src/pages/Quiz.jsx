...
function Quiz() {
  ...
  const getQuizInfo = async () => {
    await axios
      .post(`${import.meta.env.VITE_API}/getquizinfo`, { quizId: quizId }, { withCredentials: true })
      .then((res) => {
        setQuizInfo(res.data);
        setIsLoading(false);
        setPrivacy(res.data.privacy);
  ...
  const deleteQuestion = async (questionId) => {
    await axios
      .post(
        `${import.meta.env.VITE_API}/deletequestion`,
  ...
  const addQuestion = async (questionItem) => {
    await axios
      .post(
        `${import.meta.env.VITE_API}/addquestion`,
        { quizId: quizId, questionItem: questionItem },
        { withCredentials: true }
      )
      .then((res) => {
  ...
  const editQuestion = async (questionItem) => {
    await axios
      .post(
        `${import.meta.env.VITE_API}/editquestion`,
        { quizId: quizId, questionItem: questionItem },
        { withCredentials: true }
      )
      .then((res) => {
  ...
  const handleGenerate = async () => {
    if (notes.length >= 300) {
      setGenerating(true);
      await axios
        .post(`${import.meta.env.VITE_API}/generateaiquestions`, {
          quizId: quizId,
          text: notes,
  ...
  const handleSaveInEdit = async () => {
    setSaving(true);
    await axios
      .post(`${import.meta.env.VITE_API}/editquiz`, { quizId: quizId, privacy: privacy }, { withCredentials: true })
      .then((res) => {
        setSaving(false);
      });
  };
  ...
  return (
    <>
                            ...
                            <div className="accordion-body d-flex justify-content-between align-items-start">
                              <div>
                                ...
                                {item.choices.map((choice, indexChoice) => (
                                  <div
                                    className={
                                      indexChoice === item.correct_answer ? "mx-sm-0 mx-md-5 fw-bold" : "mx-sm-0 mx-md-5"
                                    }
                                    key={indexChoice}
                                  >
      ...
      <Modal show={showPopupAddQuestion}>
        <QuestionTable
          props={["Add a Question", setshowPopupAddQuestion, addQuestion, ["", "", ["", "", "", ""], -1], "Add"]}
        />
      </Modal>
      ...
      <Modal show={showPopupAutoQuestion}>
        <div
          className="d-flex flex-row justify-content-between align-items-start ps-4 pe-3 pt-3 gap-4"
          style={{ width: "100%" }}
        >
        ...
        <div className="d-flex flex-column p-4 pt-2 gap-3">
          ...
          {generating ? (
            <button
              className="btn border border-0 text-white px-2"
              style={{ backgroundColor: "#38761d" }}
              type="button"
              disabled
            >
            ...
            <button
              className={
                notes.length >= 300 ? "btn border border-0 text-white px-4" : "btn border border-0 text-white px-4 disabled"
              }
              style={{ backgroundColor: "#38761d" }}
      ...
      <Modal show={showPopupLoginFirst}>
        <div className="d-flex flex-column px-5 py-4 align-items-center gap-4" style={{ maxWidth: "400px" }}>
          ...
          <button
            ...
            onClick={() => setShowPopupLoginFirst(false)}
          >
            Continue
          </button>
        </div>
